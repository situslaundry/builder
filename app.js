...
import { 
  ...
  signOut,import { auth, db, storage } from './firebase-config.js';
import { 
  createUserWithEmailAndPassword, 
  signInWithEmailAndPassword, 
  signOut, 
...
export function showChangePasswordModal() {
  ...
  modal.innerHTML = `
    <div class="modal-card">
      <div class="modal-title">🔐 Ubah Password Akun</div>
      <div class="modal-body" style="margin-top:0.75rem;">
        <div class="form-group">
          <label>Password Baru (Minimal 6 Karakter)</label>
          <div class="password-wrapper">
            <input type="password" id="modalNewPass" class="form-control" minlength="6" placeholder="Masukkan password baru" required />
            <button type="button" class="btn-toggle-password" data-target="modalNewPass">👁️</button>
          </div>
        </div>
        <div class="form-group">
          <label>Konfirmasi Password Baru</label>
          <div class="password-wrapper">
            <input type="password" id="modalConfirmPass" class="form-control" minlength="6" placeholder="Ulangi password baru" required />
            <button type="button" class="btn-toggle-password" data-target="modalConfirmPass">👁️</button>
          </div>
        </div>
      </div>
      <div class="modal-actions">
        <button class="btn btn-sm btn-secondary btnCancel">Batal</button>
        <button class="btn btn-sm btn-primary btnSaveNewPass">Simpan Password Baru</button>
      </div>
    </div>
  `;
  container.appendChild(modal);

  // Pasang listener toggle mata
  modal.querySelectorAll('.btn-toggle-password').forEach(btn => {
    btn.onclick = (e) => {
      const targetInput = modal.querySelector(`#${e.currentTarget.dataset.target}`);
  ...
  modal.querySelector('.btnCancel').onclick = () => modal.remove();
  modal.querySelector('.btnSaveNewPass').onclick = async () => {
    const newPass = modal.querySelector('#modalNewPass').value;
    const confirmPass = modal.querySelector('#modalConfirmPass').value;
    ...
    try {
      ...
      if (err.code === 'auth/requires-recent-login') {
        showToast('Demi keamanan, silakan login ulang terlebih dahulu sebelum mengubah password.', 'warning', 5000);
      } else {
...
// Helper pasang toggle password visibility pada form
...
function renderNavbar() {
    ...
    <header class="navbar">
      ...
      <nav class="nav-links">
        ${currentUser ? `
          ...
          <button id="btnNavChangePass" class="btn btn-sm btn-secondary" style="margin-left:0.5rem;">🔑 Ubah Password</button>
  ...
  document.getElementById('btnNavChangePass')?.addEventListener('click', () => {
...
// 1. Registrasi (Dengan Toggle Password & Math Captcha)
    ...
    <div class="card" style="max-width:460px; margin: 2rem auto;">
      ...
      <form id="formRegister" style="margin-top:1.25rem;">
        ...
        <div class="form-group">
          ...
          <div class="password-wrapper">
            <input type="password" id="regPass" class="form-control" minlength="6" placeholder="Minimal 6 karakter" required />
            <button type="button" class="btn-toggle-password" data-target="regPass" title="Lihat/Sembunyikan Password">👁️</button>
          </div>
        </div>
        <div class="form-group">
...
// 2. Login (Dengan Toggle Password & Link Lupa Password)
      ...
      <form id="formLogin" style="margin-top:1rem;">
        ...
        <div class="form-group">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.35rem;">
            ...
            <a href="#/forgot-password" style="font-size:0.8rem; color:var(--primary); text-decoration:none;">Lupa Password?</a>
          </div>
          <div class="password-wrapper">
            <input type="password" id="logPass" class="form-control" required />
            <button type="button" class="btn-toggle-password" data-target="logPass" title="Lihat/Sembunyikan Password">👁️</button>
          </div>
        </div>
...
// 3. Lupa Password (Send Password Reset Email)
function renderForgotPassword() {
  ...
  app.innerHTML = `
    <div class="card" style="max-width:420px; margin: 2rem auto;">
      ...
      <p class="help-text" style="margin-top:0.35rem;">
        Masukkan alamat email akun Anda. Kami akan mengirimkan tautan aman ke email Anda untuk membuat password baru.
      </p>

      <form id="formForgotPassword" style="margin-top:1.25rem;">
        <div class="form-group">
          <label>Alamat Email Terdaftar</label>
          <input type="email" id="resetEmail" class="form-control" placeholder="nama@email.com" required />
        </div>
        <button type="submit" id="btnSendReset" class="btn btn-primary" style="width:100%;">
          📧 Kirim Petunjuk Reset Password
        </button>
        <div style="text-align:center; margin-top:1rem; font-size:0.875rem;">
          <a href="#/login" style="color:var(--text-muted); text-decoration:none;">← Kembali ke Halaman Login</a>
        </div>
      </form>
    </div>
  `;
  ...
  document.getElementById('formForgotPassword').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      showToast(`Tautan reset password berhasil dikirim ke ${email}. Silakan periksa kotak masuk/spam email Anda.`, 'success', 5000);
      setTimeout(() => {
        window.location.hash = '#/login';
      }, 2500);
    } catch (err) {
      btn.disabled = false;
      btn.innerText = '📧 Kirim Petunjuk Reset Password';
...
async function renderDashboard() {
  ...
  app.innerHTML = `
    <div class="card">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.5rem;">
        ...
        <button id="btnDashChangePass" class="btn btn-sm btn-secondary">🔐 Ubah Password Akun</button>
      </div>
  ...
  document.getElementById('btnDashChangePass')?.addEventListener('click', () => {
    showChangePasswordModal();
  });
  ...
  app.innerHTML = `
    <div class="card">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.5rem;">
        ...
        <div>
          <button id="btnAdminChangePass" class="btn btn-sm btn-secondary">🔐 Ubah Password Admin</button>
...
// ========================================================
// 7. PUBLIC LANDING PAGE & CART ENGINE
// ========================================================
